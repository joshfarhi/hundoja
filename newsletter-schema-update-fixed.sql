...
SET phone = CASE 
    WHEN phone IS NOT NULL AND ranked_duplicates.rn > 1 
    THEN phone || '_dup_' || ranked_duplicates.rn 
    ELSE phone 
END
...
-- Clean up the temporary duplicates (optional - you can review them first)
-- Uncomment the lines below if you want to remove the renamed duplicates
-- DELETE FROM newsletter_subscribers WHERE phone LIKE '%_dup_%';
